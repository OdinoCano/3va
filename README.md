...
- **Supply-chain detection gaps**: no dependency-confusion protection (scoped vs. unscoped resolution preference), no npm provenance/Sigstore signature verification. Typosquatting detection (edit-distance against an embedded popular-packages list, warned during `install`), tarball integrity (SHA-256/512) and lifecycle-script blocking are implemented; these are not.
