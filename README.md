...
⁴ Per-module breakdown in [`docs/12-roadmap/04-compatibility.md`](docs/12-roadmap/04-compatibility.md) — self-reported, not measured against an exhaustive conformance suite. One exception: `dgram` and `dns` are checked by [`scripts/compat-dgram-dns.sh`](scripts/compat-dgram-dns.sh) in CI (`compat-dgram-dns` job in [`ci.yml`](.github/workflows/ci.yml)), which runs behavioral cases against a real `node` binary and requires byte-identical stdout — not self-reported for those two modules.
