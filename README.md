...
| Feature | Node.js 24 | Bun 1.3 | **3va 2.7.0** |
|---|---|---|---|
| JavaScript runtime | ✓ | ✓ | ✓ |
...
| ECMAScript (test262) conformance⁷ | ~100%, self-reported (is V8) | ~99.9%, self-reported (JavaScriptCore) | <!--BENCH:test262-3va-->pending — filled by the next Benchmark CI run<!--/BENCH:test262-3va--> |
| Platforms | Linux/macOS/Windows/Android | Linux/macOS/Windows | Linux/macOS/Windows⁵ |
| License | MIT | MIT | MIT |
| First release → latest | 2009 → 24.x | 2023 → 1.3 | [v1.0.0](https://github.com/OdinoCano/3va/releases/tag/v1.0.0) → [v2.7.0](https://github.com/OdinoCano/3va/releases/tag/v2.7.0) |
...
⁷ tc39/test262, the official ECMA-262 conformance suite. 3va's number is the only one of the three actually measured by this repo's CI (`crates/test/src/test262.rs`, all ~53k tests across `language/`, `built-ins/`, and `intl402/` — see [`docs/09-testing/06-test262.md`](docs/09-testing/06-test262.md) for the category breakdown and known gaps: module-record-only semantics needing native `v8::Module`, `$262.agent`/`createRealm` edge cases, and ICU/CLDR data-version gaps in `intl402/`). Node's and Bun's figures are their own public self-reported numbers (V8's and JavaScriptCore's own test262 CI, respectively) — not run by this repo, so not held to the "CI-measured" bar the rest of this table is.
