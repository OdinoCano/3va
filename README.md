...
All performance rows are release builds (`cargo build --release`), all three runtimes measured together in the same run, **generated by CI, not pasted in by hand** — [`.github/workflows/benchmark.yml`](.github/workflows/benchmark.yml) runs [`bench/run.sh`](bench/) on a GitHub-hosted runner (4 vCPU, 15 GiB RAM) and publishes the table straight to the job summary, so the exact numbers above are independently reproducible from [this run](https://github.com/OdinoCano/3va/actions/runs/32180248410) — no dedicated hardware, no hand-editing. Run it yourself with `bash bench/run.sh` (see [`bench/README.md`](bench/README.md) for methodology and what each number does and doesn't mean); GitHub's shared runners are weaker and noisier than a dedicated machine, so absolute numbers will vary run to run, but the ranking has been consistent across repeated CI runs.
